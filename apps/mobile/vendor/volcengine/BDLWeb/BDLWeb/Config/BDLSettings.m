...
@implementation BDLSettings

- (instancetype)init {
    self = [super init];
    if (self) {
        _pipType = BDLSettingPiPTypeInAppAndSystemPiP;
        _enableSystemPiP = YES;
        _enableInAppPiP = YES;
        _enableAutoPlay = YES;
        _fullscreenBtnType = BDLSettingsFullscreenBtnTypeRotate;
    }
    return self;
}

- (void)setPipType:(BDLSettingPiPType)pipType {
    _pipType = pipType;
    _enableSystemPiP = _pipType == BDLSettingPiPTypeInAppAndSystemPiP;
    _enableInAppPiP = _pipType != BDLSettingPiPTypeNone;
}

+ (instancetype)sharedInstance {
    static dispatch_once_t onceToken;
    static BDLSettings *settings;
    dispatch_once(&onceToken, ^{
        settings = [[BDLSettings alloc] init];
    });
    return settings;
}
